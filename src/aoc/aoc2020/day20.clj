...
(->> (images)
     (map s/split-lines)
     (map (fn [[header & img]]
            {:id    (toInt (first (re-seq #"[\d]+" header)))
             :image (mapv vec img)
...
(defn all-image-positions [{:keys [id image edges]}]
  (->> (all-positions image)
       (map #(merge {:id id
                     :edges edges
                     :image %}
      ...
      (if (= x y 11)
        ...
        (if (< y 11)
          (let [m         (first (filter #(get (:edges %) top) tiles))
                next-tile (->> (all-image-positions m)
                               (filter #(= (:bottom %) top))
                               first)]
            (when next-tile
...
(defn monster-offsets []
  (map offsets (all-positions monster)))
...
;;(map (partial apply str) (map (partial take 60) (take 100 (picture))))
    (->>  (for [x (range (count (first picture)))
                y (range (count picture))]
              (->> monsters
                   (map (fn [monster]
                          (let [pos (map (fn [[a b]] [(+ a x) (+ b y)]) monster)]
                            (when (every? #(= \# (get-in picture %)) pos)
                              (into #{} pos)))))
                   (remove nil?)
